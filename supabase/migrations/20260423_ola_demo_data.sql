...
BEGIN
  ...
  VALUES
    ('Reunião com Carlos Mendonça',
     ...
     (now() + interval '2 days')::date, 'pending', v_l1, v_user_id, v_client_id, v_tenant_id, now()),
    ('Enviar contrato ajustado - ImobPrime',
     'Ajustar cláusula de integração com portais imobiliários.',
     now()::date, 'pending', v_l5, v_user_id, v_client_id, v_tenant_id, now()),
    ('Follow-up Fernanda - TechSolutions',
     'Verificar se recebeu a proposta.',
     (now() + interval '1 day')::date, 'pending', v_l2, v_user_id, v_client_id, v_tenant_id, now()),
    ('Ligar para Ricardo - Farmácia Saúde+',
     'Retornar contato após envio de material.',
     now()::date, 'pending', v_l3, v_user_id, v_client_id, v_tenant_id, now()),
    ('Agendar demo - GovTech',
     'Eduardo pediu demo técnica com equipe de TI.',
     (now() + interval '3 days')::date, 'pending', v_l11, v_user_id, v_client_id, v_tenant_id, now()),
    ('Onboarding LogiExpress',
     'Agendar chamada de onboarding com Thiago.',
     (now() + interval '1 day')::date, 'pending', v_l9, v_user_id, v_client_id, v_tenant_id, now()),
    ('Enviar proposta - EducaMais',
     'Amanda aguarda proposta do plano Growth.',
     (now() - interval '1 day')::date, 'completed', v_l6, v_user_id, v_client_id, v_tenant_id, now() - interval '2 days'),
    ('Qualificação - AgroMarcas',
     'Primeiro contato com Marcos.',
     (now() + interval '1 day')::date, 'pending', v_l7, v_user_id, v_client_id, v_tenant_id, now()),
    ('Análise de perda - Varejo DA',
     'Documentar motivo da perda.',
     (now() - interval '14 days')::date, 'completed', v_l10, v_user_id, v_client_id, v_tenant_id, now() - interval '15 days');
