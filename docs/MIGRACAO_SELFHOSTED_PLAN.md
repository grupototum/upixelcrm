...
- [x] **[AGENTE]** Inventariar e documentar aqui (parte que dá para fazer só com o repo, sem acesso ao cloud — ver nota de rede):
  ...
  - [x] **Cron jobs (`pg_cron`).** Confirmado via MCP do Supabase (`select * from cron.job`, 2026-08-13): **4 jobs ativos** em produção. Só um tinha migration no repo (`automation-worker`, `* * * * *`, `20260428_automation_worker_cron.sql` + fix em `20260618_automation_worker_cron_fix.sql`). Os outros 3 rodam direto do SQL Editor/painel, sem migration: `whatsapp-queue-processor` (`*/2 * * * *` — o "cron de referência já funcional" citado no fix de 20260618, agora reconciliado em `9999_reconcile_drift.sql`), `whatsapp-health-check` (`*/5 * * * *`), `whatsapp-dedup-cleanup` (`0 2 * * *`, `DELETE FROM whatsapp_message_dedup WHERE processed_at < now() - interval '7 days'` — não é HTTP, roda direto no Postgres). Todos usam o mesmo padrão `net.http_post` com Bearer fixo (exceto o dedup-cleanup). **Ainda faltam migrations de reconciliação para `whatsapp-health-check` e `whatsapp-dedup-cleanup`** — só `whatsapp-queue-processor` foi coberto nesta sessão.
...
| 2026-08-13 | 0 | Sessão Sonnet 5: MCP do Supabase autorizado nesta sessão (`list_projects`/`list_migrations`/`execute_sql` funcionando — rede/OAuth que bloqueava em 2026-07-30 não é mais um problema aqui). (1) Fora do escopo deste plano mas mesma causa-raiz: confirmado e corrigido drift de `import_logs`/`leads_import_batch_id` (migrations registradas em prod sob versão nunca versionada no git) — commit `a2fca51`, já mergeado em `main` e pushado. (2) Rodado `select * from cron.job`: confirmado `whatsapp-queue-processor` ativo, schedule `*/2 * * * *`, mais os 3 jobs já conhecidos (`automation-worker`, `whatsapp-dedup-cleanup`, `whatsapp-health-check`) — todos `active`. (3) Reescrito `9999_reconcile_drift.sql` com `cron.schedule(...)` real para `whatsapp-queue-processor`, seguindo o padrão secret-free de `20260618_automation_worker_cron_fix.sql` (config `app.supabase_url`/`app.service_role_key`, falha explícita em `RAISE EXCEPTION` se ausente — nenhum secret tocou o git). **Não executado** contra nenhum Postgres ainda. | Falta: aplicar de fato esta migration (setar `app.supabase_url`/`app.service_role_key` e rodar) numa sessão com aceite pra isso; diff completo contra schema-cloud.sql; dump do `[OPERADOR]`; demais pendências de Fase 0 inalteradas (ver linhas acima) |
