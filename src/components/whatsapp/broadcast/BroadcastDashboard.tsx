import { CreditCard, TrendingUp, BarChart, Zap, Clock, Loader2, History } from "lucide-react";
import { CostSimulator } from "./CostSimulator";
import { RechargeModal } from "./RechargeModal";
import { useState } from "react";

export function BroadcastDashboard({ credits, loadingCredits }: { credits: number, loadingCredits?: boolean }) {
  const [rechargeOpen, setRechargeOpen] = useState(false);

  // Placeholders honestos enquanto métricas reais de disparo não estão sendo agregadas no banco.
  const stats = [
    { label: "Total Enviado", icon: Zap, color: "text-primary bg-primary/10" },
    { label: "Custo Mensal", icon: TrendingUp, color: "text-success bg-success/10" },
    { label: "Uso Médio", icon: BarChart, color: "text-accent bg-accent/10" },
  ];

  return (
    <div className="space-y-6">
      {/* Top Banner / Credits */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="md:col-span-1 bg-primary text-primary-foreground rounded-card p-6 shadow-lg shadow-primary/20 flex flex-col justify-between relative overflow-hidden group">
          <CreditCard className="h-24 w-24 absolute -bottom-4 -right-4 text-white opacity-10 group-hover:rotate-12 transition-transform duration-500" />
          <div className="space-y-1">
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-80">Saldo Atual</p>
            <div className="flex items-center gap-2">
              <h2 className="text-4xl font-heading font-black tracking-tighter">
                {loadingCredits ? <Loader2 className="h-8 w-8 animate-spin" /> : credits.toLocaleString()}
              </h2>
            </div>
          </div>
          <div className="pt-6 space-y-4">
            <p className="text-[10px] font-medium opacity-70">
              {credits > 0 ? `Suficiente para ~${Math.floor(credits)} envios pagos` : "Você não possui créditos para disparos pagos."}
            </p>
            <button 
              onClick={() => setRechargeOpen(true)}
              className="w-full h-10 bg-white/20 hover:bg-white/30 rounded-xl text-xs font-bold transition-all border border-white/20 active:scale-95"
            >
              Recarregar Créditos
            </button>
          </div>
        </div>

        <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {stats.map((stat, i) => (
            <div key={i} className="bg-card ghost-border rounded-xl p-5 space-y-4 hover:shadow-card transition-all">
              <div className="flex items-center justify-between">
                <div className={`h-10 w-10 rounded-xl ${stat.color} flex items-center justify-center`}>
                  <stat.icon className="h-5 w-5" />
                </div>
              </div>
              <div>
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{stat.label}</p>
                <p className="text-xl font-heading font-black text-muted-foreground">—</p>
                <p className="text-[10px] text-muted-foreground/70 mt-1">Em breve</p>
              </div>
            </div>
          ))}
        </div>
      </div>

      <RechargeModal open={rechargeOpen} onOpenChange={setRechargeOpen} />

      {/* Main Content Area */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          {/* Recent Campaigns — empty state until aggregation backend exists */}
          <div className="bg-card ghost-border rounded-xl overflow-hidden shadow-card">
            <div className="p-4 border-b border-[hsl(var(--border-strong))] flex items-center justify-between">
              <h3 className="text-xs font-black uppercase tracking-tight text-foreground flex items-center gap-2">
                <Clock className="h-3.5 w-3.5 text-primary" /> Histórico de Disparos
              </h3>
            </div>
            <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
              <div className="h-12 w-12 rounded-full bg-muted/40 flex items-center justify-center mb-3">
                <History className="h-6 w-6 text-muted-foreground/60" />
              </div>
              <p className="text-sm font-semibold text-foreground">Nenhum disparo realizado</p>
              <p className="text-xs text-muted-foreground mt-1 max-w-xs">
                Quando você executar campanhas, o histórico de envios aparecerá aqui.
              </p>
            </div>
          </div>
        </div>

        <div className="lg:col-span-1">
          <CostSimulator />
        </div>
      </div>
    </div>
  );
}
